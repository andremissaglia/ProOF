 ...
 * @author marcio
 */
public class HCCQSPFull_time extends CplexFull{
...
//    
    private final double total_time = 60.0;                 //60 seconds
    
    private final int eventT = 5;                           //eT    = 5
    private final double eventDt = total_time/eventT;       //eDt   = 12 seconds
    
    private final int eventFactor = 3;                              
    private final int actionT = eventT*eventFactor;         //aT    = 15
    private final double actionDt = total_time/actionT;    //aDt   = 4 seconds
    
    private final int actionFactor = 2;
    private final int controlT = 30;//actionT*actionFactor;      //cT    = 30
    private final double controlDt = total_time/controlT; //cDt   = 2 seconds

    
    private final int stayOutT = 20;      //valor de [2 até controlT] que define o número de manobras, ou mudanças de lado que pode acontecer para regiões stayout 
    
    //private final int stayOutFactor = 2;                    
    //private final int stayOutT = eventT*stayOutFactor;      //sT    = 10
    //private final double stayOutDt = eventDt/stayOutFactor; //sT    = 6 seconds
    ...
    private final double d_ub[] = new double[]{total_time,      8, 8,   5, 5};
    ...
    public HCCQSPFull_time() throws IloException {}
    ...
    private final int actionC(int t){
        return t*actionFactor;
    }
    private final int controlA(int t){
        return t/actionFactor;
    }
    private final int controlE(int t){
        return t/(eventFactor*actionFactor);
    }
    
    private final int eventC(int t){
        return t*eventFactor*actionFactor;
    }
    private final int eventA(int t){
        return t*eventFactor;
    }
    ...
    public void model() throws Exception {
        ...
        x = cpx.numVarArray(controlT+1, 4, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, "x");
        u = cpx.numVarArray(controlT, 2, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, "u");        
        l = cpx.boolVarArray(actionT+1, 1+M.set("forest").length, "l");
        
        Hnh = cpx.boolVarArray(N, H, "H");
        Dnh = cpx.intVarArray(N, H, 0, actionT, "D");
        //TODO modificar no PDF, tempo máximo Tn<=time
        Tn = cpx.numVarArray(N+1, 0, total_time, "Tn");
        
        
        Zht = cpx.boolVarArray(H, actionT+1, "Z");
        ZhtS = cpx.numVarArray(H, actionT+1, 0, 1, "ZS");
        ZhtE = cpx.numVarArray(H, actionT+1, 0, 1, "ZE");
        
        
        Te = cpx.numVarArray(E, 0, total_time, "Te");
        Ke = cpx.intVarArray(E-1, 0, eventT, "Ke");
        Yst = cpx.boolVarArray(S, eventT+1, "Y");
        //========================================= Teste ============================================
...
//        }
        ...
        for(int n=0; n<N; n++){
            for(int h=0; h<H; h++){
                cpx.addGe(cpx.prod(Dnh[n][h], actionDt), cpx.prod(Hnh[n][h], d_lb[h]), "4.66Ge");
                cpx.addLe(cpx.prod(Dnh[n][h], actionDt), cpx.prod(Hnh[n][h], d_ub[h]), "4.66Le");
            }
        }
        ...
        for(int n=0; n<N; n++){
            for(int h=0; h<H; h++){
                Dnh[n][h].setUB((int)(d_ub[h]/actionDt+1e-6));
            }
        }
        ...
        for(int n=0; n<N; n++){
            IloNumExpr sum = Tn[n];
            for(int h=0; h<H; h++){
                sum = cpx.SumProd(sum, actionDt, Dnh[n][h]);
            }
        ...
        //(4.68)
        for(int n=0; n<N; n++){
            for(int t=0; t<actionT+1; t++){
                IloNumVar y = cpx.IF_BetweenLT_Them_Y("(4.68.Betwenn)", Tn[n], t*actionDt, Tn[n+1]);
                for(int h=0; h<H; h++){
        ...
        for(int h=0; h<H; h++){
            for(int t=0; t<actionT+1; t++){
                cpx.addIF_Lt_Them_Y("(4.69)", cpx.Not(Zht[h][t]), cpx.sum(Tn[N], -t*actionDt));
            }
        }
        ...
        for(int t=0; t<actionT+1; t++){
            ...
            //TODO modificar no PDF
            for(int i=0; t<actionT && i<x0.length; i++){
                cpx.addIF_Y_Them_Eq("(4.77).p", cpx.Not(sum), cpx.sum(x[actionC(t+1)][i], cpx.prod(-1, x[actionC(t)][i]))); 
            }
        }
        ...
        for(int h=0; h<H; h++){
            for(int t=0; t<actionT+1; t++){
                if(h==1 || h==2){   //fill
                    ...
                    cpx.addIF_Y_Them_StayIn("(4.76)", Zht[h][t], hyperplanes, new double[hyperplanes.length], x[actionC(t)]);
...
//        }
        ...
        for(int h=0; h<H; h++){
            for(int t=0; t<controlT; t++){
                if(h==0){   
                    ...
                    IloNumExpr expr[] = new IloNumExpr[4];
                    expr[0] = cpx.sum(cpx.prod(-1, x[t+1][0]), x[t][0], cpx.prod(controlDt, x[t][2]),  cpx.prod(controlDt*controlDt/2, u[t][0]));
                    expr[1] = cpx.sum(cpx.prod(-1, x[t+1][1]), x[t][1], cpx.prod(controlDt, x[t][3]),  cpx.prod(controlDt*controlDt/2, u[t][1]));
                    expr[2] = cpx.sum(cpx.prod(-1, x[t+1][2]),          x[t][2],                cpx.prod(controlDt, u[t][0]));
                    expr[3] = cpx.sum(cpx.prod(-1, x[t+1][3]),          x[t][3],                cpx.prod(controlDt, u[t][1]));
                    cpx.addIF_Y_Them_Eq("(4.57)", Zht[h][controlA(t)], expr);
                    
                    
                    cpx.addIF_Y_Them_Le("(4.58).v_max", Zht[h][controlA(t)], cpx.sum(x[t][2], -v_max), cpx.sum(x[t][3], -v_max));
                    cpx.addIF_Y_Them_Ge("(4.58).v_min", Zht[h][controlA(t)], cpx.sum(x[t][2], +v_max), cpx.sum(x[t][3], +v_max));
                    
                    cpx.addIF_Y_Them_Le("(4.58).u_max", Zht[h][controlA(t)], cpx.sum(u[t][0], -u_max), cpx.sum(u[t][1], -u_max));
                    cpx.addIF_Y_Them_Ge("(4.58).u_min", Zht[h][controlA(t)], cpx.sum(u[t][0], +u_max), cpx.sum(u[t][1], +u_max));
                }else{
                    //Dynamic to STAY
                    ...
                    //      Zht --> |v(t+1)| = 0                    (4.77).v
                    //      Zht --> |u(t)| = 0                      (4.77).u
                    cpx.addIF_Y_Them_Eq("(4.77).p", Zht[h][controlA(t)],
                            cpx.sum(x[t+1][0], cpx.prod(-1, x[t][0])), 
                            cpx.sum(x[t+1][1], cpx.prod(-1, x[t][1])));
                    cpx.addIF_Y_Them_Eq("(4.77).v", Zht[h][controlA(t)], x[t][2], x[t][3]);
                    cpx.addIF_Y_Them_Eq("(4.77).v", Zht[h][controlA(t)], x[t+1][2], x[t+1][3]);
                    cpx.addIF_Y_Them_Eq("(4.77).u", Zht[h][controlA(t)], u[t][0], u[t][1]);
                }
            }
        }
        ...
        //(4.86)
        for(int e=0; e<E-1; e++){
            cpx.addEq(Te[e+1], cpx.sum(Te[e], cpx.prod(eventDt, Ke[e])), "4.86");
        }
        ...
        for(int t=0; t<eventT+1; t++){
            cpx.addIF_Eq_Them_Y("(4.89)", Yst[0][t], cpx.sum(Te[1], -t*eventDt));    //not fire1 and not fire2
            cpx.addIF_Eq_Them_Y("(4.89)", Yst[1][t], cpx.sum(Te[2], -t*eventDt));    //go base
        }
        ...
        for(int t=0; t<eventT+1; t++){
            cpx.addIF_Between_Them_Y("(4.90)", Yst[2][t], Te[0], t*eventDt, Te[2]);
        }
        
        
        //(4.92)    //literals
        for(int t=0; t<eventT+1; t++){
            cpx.addIF_Y_Them_Z_Eq_W("(4.92)", Yst[0][t], l[eventA(t)][1], false);   //Yst -> fire1 = false
            cpx.addIF_Y_Them_Z_Eq_W("(4.92)", Yst[0][t], l[eventA(t)][2], false);   //Yst -> fire2 = false
        }
        
        
        //(4.91)
        for(int t=0; t<controlT+1; t++){   //stay in
            cpx.addIF_Y_Them_StayIn("(4.91)", Yst[1][controlE(t)], M.set("base")[0].hyperplans, null, x[t]);    //go base
        }

        for(Map.Region r : M.set("NFZ")){    //stay out
            cpx.addIF_Y_Them_StayOut("(4.91)", Yst[2], r.hyperplans, null, x, stayOutT);    
        }
    }
    ...
    public void print() throws Exception {
        ...
        String timesA[] = new String[actionT+1];
        for(int t=0; t<actionT+1; t++){
            timesA[t] = "t="+(int)(t*actionDt);
        }
        double v_Zht[][] = cpx.getValues(Zht, true);
        cpx.print("Zht", "%15g", v_Zht, "%15s", timesA);
        ...
        for(int t=0; t<controlT+1; t++){
            String actions[] = new String[]{"fly", "fill(lake1)", "fill(lake2)", "drop(forest1)", "drop(forest2)", "empty"};
            int action = H;
            double max = 0.01;
            for(int h=0; h<H; h++){
                if(v_Zht[h][controlA(t)]>max){
                    max = v_Zht[h][controlA(t)];
                    action = h;
                }
            }
            System.out.printf("%15s %15g %15g %15g %15g %15g %15g %15s %15g %15g %15g\n", 
                    "t = "+((int)(t*controlDt*100))/100, v_x[t][0], v_x[t][1], v_x[t][2], v_x[t][3], t<actionT?v_u[t][0]:0, t<controlT?v_u[t][1]:0, actions[action], v_l[controlA(t)][0], v_l[controlA(t)][1], v_l[controlA(t)][2]);
        }
        
        double v_Te[] = cpx.getValues(Te, true);
        cpx.print("Te", "%15g", v_Te);
        
        String timesE[] = new String[eventT+1];
        for(int t=0; t<eventT+1; t++){
            timesE[t] = "t="+(int)(t*eventDt);
        }
        double v_Yst[][] = cpx.getValues(Yst, true);
        cpx.print("Yst", "%15g", v_Yst, "%15s", timesE);
    ...
    public static void main(String[] args) throws IloException, Exception {
        ...
        HCCQSPFull_time hccqsp = new HCCQSPFull_time();
        hccqsp.model();
