 ...
 * @param <App>
 */
public abstract class UAVModel <App extends LinearSystem>{
    public enum Callback {
        PARTIAL_INCUMBENT, PARTIAL_OPTIMAL, PARTIAL_FEASIBLE, OPTIMAL, FEASIBLE, FINAL_EXTRACT
    }
    
    public final String name;
    public final App system;
    public final CplexExtended cplex;
    public final IncumbentBest inc;
    
    public UAVModel(String name, App app, CplexExtended cplex) throws IloException, InstantiationException, IllegalAccessException {
        this.name = name;
        this.system = app;
        this.cplex = cplex;
        ...
        this.inc = new IncumbentBest() { 
            ...
            public void main2() throws Exception {
                ...
                system.solutionCallback(this, THIS, Callback.PARTIAL_INCUMBENT); 
            }
        };
        this.cplex.use(inc); 
    }
    ...
    public boolean equals(Object obj) {
        return name.equals(((UAVModel)obj).name); //To change body of generated methods, choose Tools | Templates.
    }
    
    private boolean print = false;
    public boolean solve(double time, double epGap, int threads, boolean print_war, boolean print_out) throws Exception {
        this.print = print_out;
    ...
    public boolean solve() throws Exception {
        ...
        if(print) System.out.printf("Frist solve '%s', current status = '%s'\n", name, status.toString());
        ...
        if(!cplex.solve()) {
            ...
            time_cplex = -1;
            status = cplex.getStatus();
            if(print)System.out.printf("Cplex doesn't solve the model, status = '%s'\n", status.toString());
            return false;
        }else{
            time_cplex = cplex.getCplexTime();
            if(cplex.getStatus()==Status.Optimal){
                status = Status.Optimal;
                //model_status = Callback.PARTIAL_OPTIMAL;
                system.solutionCallback(cplex, this, model_status);
            }else if(cplex.getStatus()==Status.Feasible){
                status = Status.Feasible;
                //model_status = Callback.PARTIAL_FEASIBLE;
                system.solutionCallback(cplex, this, model_status);
            }else{
                throw new Exception("Cplex invalid status '"+cplex.getStatus().toString()+"' to solve() = true");
            }
            if(print)System.out.printf("Status cplex after solve = '%s' and partial status = '%s'\n", cplex.getStatus().toString(), status.toString());

            iteration = 1;
            while(addChanges()){
                if(print)System.out.printf("addChanges ocurr, iteration = '%d'\n", iteration);
                ...
                if(remaining_time>ti_lim*0.05){//I have enough time
                    ...
                    if(print)System.out.printf("Again [%d] solve '%s' for more %g time seconds, partial status ='%s'\n", iteration, name, remaining_time, status.toString());
                    if(cplex.solve()){
                        if(cplex.getStatus()==Status.Optimal){
                    ...
                    if(print)System.out.printf("Stop [%d] with unknown solution because it doesn't have time to solve '%s' again after changes\n", iteration, name);
                    break;
                }
            }
            time_tot = time_solve();
            if(status == Status.Optimal){
                model_status = Callback.OPTIMAL;
                system.solutionCallback(cplex, this, model_status);
                return true;
            }else if(status == Status.Feasible){
                model_status = Callback.FEASIBLE;
                system.solutionCallback(cplex, this, model_status);
                return true;
            }else{
                return false;
    ...
    public final Status cplex_status() throws IloException{
        return status;
    }
    public final Callback model_status(){
        return model_status;
    }
    ...
    public double lower() throws IloException {
        return cplex.getNbinVars()==0 ? cplex.getObjValue() : cplex.getBestObjValue();
    }
    public void results(LinkerResults link) throws Exception {
        link.writeString(name+"-status", status==null? "null" :status.toString());
        if(isFeasible()){
            ...
            link.writeDbl(name+"-time-cplex", time_cplex);
            link.writeInt(name+"-uav-iteration", iteration);
            system.solutionCallback(cplex, this, Callback.FINAL_EXTRACT);
        }
    }
}
