...
//        
    ...
    public static void addRestrictionsJPL(CplexExtended cplex, IloIntVar Zi[][], IloNumVar Xt[][], final double bigM, final double R) throws IloException{
        //-------------------------------- (36) --------------------------------
        for(int t=1; t<Zi.length; t++){
            for(int i=0; i<N; i++){
                IloNumExpr aux = cplex.sum(cplex.prod(-ax(i), Xt[t-1][0]),cplex.prod(-ay(i), Xt[t-1][1]));
                aux = cplex.sum(aux, cplex.prod(bigM, Zi[t][i]));

                double val = bigM - b(i) - R;
                cplex.addLe(aux, val, "JPL."+(t+1)+","+(i+1)+"");
            }
        }
        for(int t=0; t<Zi.length; t++){
            IloNumExpr exp = null;
    ...
    public static void main1(String[] args) throws Exception {
        ...
        switch(REST){
            ...
            case 7: addRestrictionsJPL(cplex, Zi, Xt, bigM, R);    break;
        }
    ...
    public static void save(JPanel draw, double resolution){
        BufferedImage img = new BufferedImage(
                (int)(draw.getWidth()*resolution), (int)(draw.getHeight()*resolution), 
        ...
        try {
            draw.paint(g);
            ImageIO.write(img, "png", new File("./img"+N+""+TEST+""+REST+""+MODE+".png"));
        } catch (Throwable ex) {
            ex.printStackTrace();
        }
    }
