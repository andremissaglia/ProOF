 ...
 * @author marcio
 */
public abstract class CplexNumFunctionCond implements iCplexFunction1v{
    private final IloNumVar v[];
    public final IloNumVar x;
    public final IloNumVar f;
    public final int N;
    public final String name;
    public CplexNumFunctionCond(CplexExtended cplex, int N, IloNumVar y, boolean bounded, String name) throws IloException {
        this.N = N;
        this.name = name;
        v = cplex.numVarArray(N, 0, Double.POSITIVE_INFINITY, name+".v");
        if(bounded){
//            x = cplex.numVar(0, x(N), name+".x");
//            f = cplex.numVar(0, f(N), name+".f");
            x = cplex.numVar(x(0), Double.POSITIVE_INFINITY, name+".x");
            f = cplex.numVar(f(0), Double.POSITIVE_INFINITY, name+".f");
//            x = cplex.numVar(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, name+".x");
//            f = cplex.numVar(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, name+".f");
            for(int n=0; n<N; n++){
                v[n].setUB(dx(n));
            }
            for(int n=0; n<N; n++){
                cplex.addLe(v[n], cplex.prod(dx(n), y), name+".ub{v}");
            }
        }else{
            x = cplex.numVar(x(0), Double.POSITIVE_INFINITY, name+".x");
            f = cplex.numVar(f(0), Double.POSITIVE_INFINITY, name+".f");
            ...
            cplex.addLe(v[N-1], cplex.prod(1e5, y), name+".ub{v}");
        }
