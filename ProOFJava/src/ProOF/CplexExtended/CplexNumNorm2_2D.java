...
public class CplexNumNorm2_2D {
    ...
    public CplexNumNorm2_2D(CplexExtended cplex, String name, int N, double MAX, IloNumExpr exp_x, IloNumExpr exp_f) throws IloException {
        this(cplex, name, N, MAX, 1.0, exp_x, exp_f);
    }
    public CplexNumNorm2_2D(CplexExtended cplex, String name, int N, double MAX, double coef, IloNumExpr exp_x, IloNumExpr exp_f) throws IloException {
        if(N<3){
        ...
        if(coef<0){
            throw new IloException("coef<0 is invalid");
        }
        this.name = name;
        f = cplex.numVar(0, MAX, name+".f");
        for (int n = 0; n < N; n++) {
            IloNumExpr exp = cplex.sum(
                cplex.prod(Math.cos((2 * Math.PI * n)*coef / N), exp_x), 
                cplex.prod(Math.sin((2 * Math.PI * n)*coef / N), exp_f)
            );
